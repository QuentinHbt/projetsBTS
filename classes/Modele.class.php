<?php
// Définition des identifiants de connexion à la base de données
define('DSN', 'mysql:host=mysql3.000webhost.com;dbname=a5751282_Gestage'); 
define('USER', 'a5751282_root');
define('MDP', 'j0l!verie'); 
...
abstract class Modele {
    ...
    function getAll() {
        ...
        // La classe Enregistrement est une classe générique vide qui sera automatiquement remplie d'autant
    ...
    // Fonction qui sélectionne toutes les données d'une table se rapportant à un login 
   ...
   // Fonction qui récupère des données en fonction d'un utilisateur : idSpécialité, idRole
    function getFromLoginValeursId($valeurLogin) {
        $pdo = $this->connecter();
        // Requête textuelle
        $query = "SELECT * FROM " . $this->table . " E INNER JOIN SPECIALITE O ON E.IDSPECIALITE = O.IDSPECIALITE INNER JOIN ROLE R ON R.IDROLE = E.IDROLE WHERE LOGINUTILISATEUR = '" . $valeurLogin . "'";
        $queryPrepare = $pdo->prepare($query);
        // Spécifier le type de classe à instancier
        $queryPrepare->setFetchMode(PDO::FETCH_CLASS, $this->nomClasseMetier);
        // Exécuter la requête avec les valeurs des paramètres
    ...
    // Fonction de sélection d'un id dans une table en fonction d'un libellé et d'une valeur
    ...
    // Fonction qui récupère la liste de tous les étudiants
    ...
    // Fonction qui récupère la liste de toutes les entreprises
    ...
    // fonction qui permet d'insérer un stage dans la base de données
